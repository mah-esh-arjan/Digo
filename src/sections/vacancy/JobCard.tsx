...
export default function JobCard({ id, title, type, location, date }: JobProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    body.append('vacancyId', String(id));
  ...
  return (
    <>
      <StaggerItem className="bg-white border border-gray-100 p-6 md:p-8 rounded-2xl hover:shadow-md transition-shadow flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <Button onClick={() => setIsOpen(true)} className="bg-navy hover:bg-primary rounded-xl px-8 h-12 w-full md:w-auto">
          Apply Now
        </Button>
      </StaggerItem>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
            <div className="flex justify-between items-center px-6 py-4 border-b border-slate-100">
              <div>
                <h2 className="text-xl font-bold text-slate-900">Apply for {title}</h2>
                <p className="text-sm text-slate-500">{location} · {type}</p>
              </div>
              <button onClick={() => { setIsOpen(false); setSubmitted(false); }} className="text-slate-400 hover:text-slate-600">
                <X size={24} />
              </button>
            </div>
            ...
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <button type="submit" disabled={submitting}
                  className="w-full bg-navy text-white font-semibold py-2.5 rounded-lg hover:bg-primary transition-colors disabled:opacity-60">
