    ...
    <section className="py-32 relative bg-background">
      ...
      <Container>
        <FadeIn className="text-center mb-16">
          <div className="inline-block px-4 py-1.5 rounded-full bg-primary/5 border border-primary/10 text-primary font-semibold text-sm mb-6">Specifications</div>
        ...
        <div className="max-w-4xl mx-auto relative group z-10">
          ...
          <div className="relative glass-card border border-white/60 rounded-3xl overflow-hidden shadow-2xl bg-white/80 backdrop-blur-xl transition-all duration-300">
            <StaggerGrid className="divide-y divide-gray-100/80">
              {data.map((item, i) => (
                <StaggerItem key={i} className={`group/row flex flex-col sm:flex-row justify-between p-6 md:p-8 transition-all duration-300 relative overflow-hidden ${item.highlight ? 'bg-primary/5 hover:bg-primary/10' : 'hover:bg-gray-50/50'}`}>
