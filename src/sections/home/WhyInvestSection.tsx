...
export default function WhyInvestSection() {
  return (
    <section className="py-32 bg-surface relative overflow-hidden">
      {/* Abstract Background fluid elements */}
      ...
      <Container className="relative z-10">
        ...
        <StaggerGrid className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {reasons.map((r, i) => (
            <StaggerItem 
              key={i} 
              className="group relative bg-white p-8 rounded-[2rem] border border-gray-100 hover:border-accent/30 shadow-[0_4px_20px_-10px_rgba(0,31,70,0.1)] hover:shadow-[0_20px_40px_-10px_rgba(0,224,255,0.15)] transition-all duration-500 hover:-translate-y-2 overflow-hidden flex flex-col"
            >
              {/* Card glowing background on hover */}
              ...
              {/* Icon Container with ripple animation */}
              <div className="relative w-16 h-16 rounded-2xl bg-gradient-to-br from-primary/5 to-accent/10 border border-white/50 flex items-center justify-center mb-8 shadow-inner group-hover:scale-110 transition-transform duration-500 z-10 overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-tr from-primary/20 to-accent/20 opacity-0 group-hover:opacity-100 scale-0 group-hover:scale-150 transition-all duration-700 ease-out rounded-full" />
