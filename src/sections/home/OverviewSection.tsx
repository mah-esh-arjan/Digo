      ...
      <Container className="relative z-10">
        <FadeIn className="text-center max-w-2xl mx-auto mb-20 text-balance">
          <motion.div 
            ...
            viewport={{ once: true }}
            className="inline-block px-4 py-1.5 rounded-full bg-primary/5 border border-primary/10 text-primary font-semibold text-sm mb-6"
          >
            Data Visualization
          </motion.div>
        ...
        <FadeInStagger className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat, i) => (
            <motion.div 
              ...
              className="group relative p-8 glass flex flex-col items-center text-center rounded-3xl overflow-hidden transition-all duration-500"
            >
              {/* Hover Glow Effect */}
              ...
              <div className="relative z-10">
                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-primary/10 to-accent/10 border border-white/40 text-primary flex items-center justify-center mb-6 shadow-inner relative overflow-hidden group-hover:shadow-[0_0_20px_rgba(0,224,255,0.3)] transition-all duration-300">
                  ...
                  <motion.div 
                    className="absolute inset-0 bg-gradient-to-tr from-primary/20 to-accent/20"
                ...
                <div className="text-4xl font-extrabold text-navy mb-1 group-hover:text-primary transition-colors duration-300">
                  {stat.isNumber ? (
                    <>
                      ...
                      />
                      <span className="text-2xl text-gray-400 ml-1 font-medium">{stat.suffix}</span>
                    </>
                  ) : (
                    stat.value
                  )}
